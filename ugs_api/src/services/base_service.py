...
class UGSService:
    def __init__(
        ...
        Args:
          model: модель UGSModel (Pydantic) для валидации данных;
    ...
    @staticmethod
    def _create_search_query(film_id, user_id):
    ...
    async def search(
        ...
        """GET запрос для поиска данных. Возвращает несколько значений. Можно
        фильтровать запрос по пользователю или фильму. Поддерживается пагинация
        (LIMIT / OFFSET).
    ...
    async def create(
        ...
        """CREATE запрос для создания данных. Если у этого пользователя для
        этого фильма уже была создана запись, то заменяем старую на новую.
    ...
    async def delete(self, film_id: Optional[UUID], user_id: Optional[UUID]):
        """DELETE запрос для удаления данных. На данным этапе считаем, что
        связка "пользователь + фильм" уникальна и ничего лишнего мы не удалим.

        Можно добавить проверку на наличие записи, но это лишний запрос в БД:
